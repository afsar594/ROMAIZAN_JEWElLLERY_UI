...
@Component({
  selector: 'app-leavesalaryamountpolicy',
  templateUrl: './leavesalaryamountpolicy.component.html',
  styleUrls: ['./leavesalaryamountpolicy.component.scss']
})
export class LeavesalaryamountpolicyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.masterApiService.GetAllHrLeaveSalAmtPolicyMaster()
    this.masterApiService.GetAllHrLeaveSalAmtPolicyDetails()
  ...
  onEdit() {
    ...
    this.masterApiService.UpdateHrLeaveSalAmtPolicyMaster(this.GAPForm.value).subscribe((response: any[]) => {
      this.initForm()
      this.isBtnEnable = true
      this.submitted = false
      this.masterApiService.GetAllHrLeaveSalAmtPolicyMaster()
      this.policyMasterId = response[response.length - 1].polId
      this.masterApiService.HrLeaveSalAmtPolicyMaster = response
      this.submitted = false
  ...
  onSave() {
    ...
    this.masterApiService.InsertHrLeaveSalAmtPolicyMaster(this.GAPForm.value).subscribe((response: any[]) => {
      this.initForm()
      this.isBtnEnable = true
      this.submitted = false
      this.masterApiService.GetAllHrLeaveSalAmtPolicyMaster()
  ...
  onSaveDetail() {
    ...
    this.masterApiService.InsertHrLeaveSalAmtPolicyDetails(this.detailArray).subscribe((response: any[]) => {
      this.initForm()
      this.policyMasterId = response[response.length - 1].polId
      this.masterApiService.GetAllHrLeaveSalAmtPolicyDetails()
      this.submitted = false
  ...
  ViewDetails(polId) {
    this.detailData = this.masterApiService.HrLeaveSalAmtPolicyDetails.filter(o => o.polId == polId)
    this.viewDetailModal = true
  }

  onDeleteMaster(dataObj: any) {
    if (confirm("Are you sure. You want to delete this record ?")) {
      this.masterApiService.DeleteHrLeaveSalAmtPolicyMaster(dataObj).subscribe(response => {
  ...
  onDeleteDetail(dataObj: any) {
    if (confirm("Are you sure. You want to delete this record ?")) {
      this.masterApiService.DeleteHrLeaveSalAmtPolicyDetails(dataObj).subscribe(response => {
        this.messageService.add({ severity: 'Success', summary: 'Success!', detail: 'Record Deleted Successflly!' });
